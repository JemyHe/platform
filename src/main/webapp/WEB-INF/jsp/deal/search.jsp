                ...
                <div class="row">
                    <c:forEach items="${pagingDealList.data}" var="item">
                        <div class="pro col-lg-2">
                            ...
                            <div class="operate">
                                <a class="addcart" data-id="${deal.skuId}">加入购物车</a>
        ...
        <div class="page-nav">
            <div class="container">
                <div class="page-body">
                    ...
                    <c:if test="${pagingDealList.page > 1 }">
                        <a href="${ctx}/search/${pagingDealList.page-1}/${s}" class="pre">
                            上一页
                        </a>
                    </c:if>
                    ...
                    <c:if test="${pagingDealList.totalPage==1}">
                        <a href="${ctx}/search/1/${s}"><span class="current">1</span></a>
                    </c:if>
                    <c:if test="${pagingDealList.totalPage>1}">
                        <c:forEach var="item" begin="1" end="${pagingDealList.totalPage}">
                            <c:if test="${pagingDealList.page == item}">
                                <a href="${ctx}/search/${item}/${s}"><span class="current">${item}</span></a>
                            </c:if>
                            <c:if test="${pagingDealList.page != item}">
                                <a href="${ctx}/search/${item}/${s}">${item}</a>
                            </c:if>
                        </c:forEach>
                    </c:if>
                    ...
                    <c:if test="${pagingDealList.page < pagingDealList.totalPage}">
                        <a href="${ctx}/search/${pagingDealList.page+1}/${s}" class="next">下一页</a>
                    </c:if>
